...
describe("source display metadata", () => {
  it.each(SEGMENT_LABEL_CASES)(
    "labels the %s segment",
    (segment, expectedLabel) => {
      expect(getSegmentLabel(segment)).toBe(expectedLabel)
    },
  )

  it.each(SOURCE_LABEL_CASES)("labels the %s source", (path, expectedLabel) => {
    expect(getSourceDisplayLabel(path)).toBe(expectedLabel)
  })

  it("covers every supported metadata label", () => {
    expect(SEGMENT_LABEL_CASES).toHaveLength(10)
    expect(SOURCE_LABEL_CASES).toHaveLength(60)
  })
